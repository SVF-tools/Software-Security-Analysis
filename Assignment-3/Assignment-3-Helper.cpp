...
/**
 * @brief Add a detected bug to the bug reporter
 *
 * This function adds a detected bug to the bug reporter. It gathers the relevant information
 * from the exception and the current ICFG node, constructs the event stack for the bug, and
 * records the bug if it has not been previously reported at the same location.
 *
...
void AbstractExecution::addBugToReporter(const AEException& e, const ICFGNode* node) {
    ...
    if (const CallICFGNode* call = SVFUtil::dyn_cast<CallICFGNode>(node)) {
        ...
        inst = node->getSVFStmts().back()->getInst();  // Otherwise, get the last instruction of the node's statements
    }

    GenericBug::EventStack eventStack;
    SVFBugEvent sourceInstEvent(SVFBugEvent::EventType::SourceInst, inst);

    // Build the event stack by adding call site events from the call site stack
...
/**
 * @brief Propagate the execution state if it is feasible
 *
...
bool AbstractExecution::propagateStateIfFeasible(const ICFGNode *block) {
    ...
    if (inEdgeNum == 0) {
        ...
        // If feasible states are found, update the pre-execution state of the block
...
/**
 * @brief Get the byte size of an allocation instruction
 *
 * This function calculates the byte size of a memory allocation instruction given the current abstract state
 * and an address statement. It handles both constant and variable sizes for memory objects.
 * e.g. int arr[10] => it is an AddrStmt which allocates [i32*10], call getAllocaInstByteSize(as, addr) and return 40
 *
 * @param as The current abstract state
...
u32_t AbstractExecution::getAllocaInstByteSize(AbstractState& as, const AddrStmt *addr) {
    ...
    if (const ObjVar* objvar = SVFUtil::dyn_cast<ObjVar>(addr->getRHSVar())) {
        ...
        // If the memory object has a constant byte size, return that size directly
        if (objvar->getMemObj()->isConstantByteSize()) {
            u32_t sz = objvar->getMemObj()->getByteSizeOfObj();
            return sz;
        } else {
            // Handle variable-sized memory objects
            const std::vector<SVFValue*>& sizes = addr->getArrSize();
            u32_t elementSize = 1; // Default element size is set to 1
            u64_t res = elementSize;

            // Iterate over the size values and calculate the total byte size
            for (const SVFValue* value : sizes) {
                // Check if the value node is in the variable-to-value table of the abstract state
                if (!as.inVarToValTable(_svfir->getValueNode(value))) {
                    // If not, set it to the maximum field limit interval
                    as[_svfir->getValueNode(value)] = IntervalValue(Options::MaxFieldLimit());
                }
                IntervalValue itv = as[_svfir->getValueNode(value)].getInterval();
                // Multiply the result by the upper bound of the interval, ensuring it doesn't exceed the max field limit
                res = res * itv.ub().getIntNumeral() > Options::MaxFieldLimit() ? Options::MaxFieldLimit() : res * itv.ub().getIntNumeral();
            }
            return (u32_t)res; // Return the calculated byte size
        }
    }

    // If the RHS value is not an ObjVar, assert false and abort the program
