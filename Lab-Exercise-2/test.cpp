...
int main(int argc, char **argv) {
    ...
    if (test_name == "test1") {
        ...
        result = z3Mgr->hasZ3Expr("q") &&  z3Mgr->getEvalExpr(z3Mgr->loadValue(z3Mgr->getZ3Expr("q")) == z3Mgr->getZ3Expr(10)).is_true();
    ...
    if (result == false) {
        std::cout << SVFUtil::errMsg("The test-") << SVFUtil::errMsg(test_name) << SVFUtil::errMsg(" assertion is unsatisfiable!!") << std::endl;
        assert(result);
    }
    z3Mgr->resetSolver();
    delete z3Mgr;
